describe('Archivistica Login Test', ()=>{
    ...
    it('TC1: Upload documents succesfully', ()=>{
        cy.visit("http://domus.dyndns.biz:5555/test_arch/administrator/processes/P001C")
        cy.url().should('contain', "processes")
        cy.wait(2000)
        ...
        cy.get('[class="custom-control-label mr-1 manita"]').click()
        ...
        cy.get('[class$="toast-error"]', { timeout: 1000 }).should('not.exist')
        // cy.get('span.toast-error').should('not.exist')

    })

    it.skip('TC1: Upload documents unsuccesfully', ()=>{
        cy.visit("http://domus.dyndns.biz:5555/test_arch/administrator/processes/P001C")
        cy.url().should('contain', "processes")
        cy.wait(2000)
        ...
        cy.get('div.toast-container').within(() => {
            cy.get('[class$="toast-error"]').should('contain', 'Llena todos los campos obligatorios *')
        })
